...
const twitchCapabilites = [
  {name: 'tags', ack: false},
  {name: 'membership', ack: false},
  {name: 'commands', ack: false}
];

var loginResponses = {
    '001': {res: ':Welcome, GLHF!'},
    '002': {res: ':Your host is tmi.twitch.tv'},
    '003': {res: ':This server is rather new'},
    '004': {res: ':-'},
    '375': {res: ':-'},
    //'372': {res: ':You are in a maze of twisty passages.'},            //dev docs say this
    '372': {res: ':You are in a maze of twisty passages, all alike.'},   //server sends this
    '376': {res: ':>'}
};
...
const TwitchBot = class TwitchBot extends EventEmitter {
  ...
  constructor({
    ...
    channels.forEach(channel => {
      ...
      this.channels.push(parser.formatCHANNEL(channel));
    });

    //console.log("RCJ:", responseChannelJoins);
  ...
  async afterConnect(){
    ...
    twitchCapabilites.forEach(async (capability) => {
      this.writeIrcMessage(`CAP REQ ${twitchCapPath}${capability.name}`);
      //await this.irc.once('data', data => {console.log(`CAP ACK ${capability}`,data);});
    });
      ...
      ircEvent.ircMessages.forEach((ircMessage) => {
        ...
        switch (mappedCommand) {
          ...
          case 'CAP':
            ...
            twitchCapabilites.forEach(async (capability) => {
              if (`${twitchCapPath}${capability.name}` === ircMessage.args.capability) {
                capability.ack = true;
              }
            });
            //console.log("twitchCapabilites", twitchCapabilites);
            break;
          ...
          case '366':
            ...
            //console.log("jRs: ", this.joinResponses[idx.CH][idx.CO]);
            break;
