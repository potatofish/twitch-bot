...
module.exports = {
  ...
  format__general(event) {
    ...
    */
    ...
    const tagRawString = event.charAt(0) === '@' ? toBeParsed.split(' :').shift() : '';
    ...
    ircEvent.user = toBeParsed.split("\r\n").shift();
    toBeParsed = toBeParsed.slice(ircEvent.user.length + 1); // removed the processed characters
    //console.log("userdata_added", ircEvent);

    if(tagRawString !== '') {
      // drop the first character "@" & split into array by ";"
      var tagArray = tagRawString.substr(1).split(';');
