...
module.exports = {
  ...
  format__general(event) {
    // if there are any tags split them timeout
    if (event.charAt(0) === "@") {
    ...
    */
    ...
    var tags = event.split(':').charAt(0) === '@' ? split_event[0].shift().split(';') : undefined;
    console.log("tags", tags);


    if (split_event[0].charAt(0) === "@") {
      const tagsString = split_event[0].shift().split(';')
      tags = tagsString;
      console.log("tags", tags);
    }
