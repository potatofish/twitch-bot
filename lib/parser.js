...
module.exports = {
  ...
  format__general(event) {
    ...
    function format__line(message) {
      ...
      unParsedMsg = unParsedMsg.slice(ircMessage.channel.length + 1); // removed the processed characters
      //console.log("channel_added", ircEvent);


      ircMessage.content = unParsedMsg.split("\r\n").shift();
      unParsedMsg = unParsedMsg.slice(ircMessage.content.length + 2); // removed the processed characters
      //console.log("userdata_added", ircEvent);

      console.log(`[${count}] message Processed`, ircMessage);
      console.log(`[${count}] REMAINING TO BE PARSED`,unParsedMsg);    }
    /*
    ...
    */
    ...
    if (event.charAt(0) === '@') {
      // spit off the tags as the first element divided by a space
      const tagRawString = toBeParsed.split(' ').shift();
      ...
      tagArray.forEach((tag, idx) => {
        ...
        const key = splitTag[0];
        const val = splitTag[1];
        ircEvent.tags[key] = val;
      });

      toBeParsed = toBeParsed.slice(tagRawString.length + 1); // removed the processed tag characters
    }

    //console.log("tags_added", ircEvent);

    ircEvent.clientPrefix = toBeParsed.split(" ").shift();
    toBeParsed = toBeParsed.slice(ircEvent.clientPrefix.length + 1); // removed the processed characters
    //console.log("prefix_added", ircEvent);


    ircEvent.command = toBeParsed.split(" ").shift();
    toBeParsed = toBeParsed.slice(ircEvent.command.length + 1); // removed the processed characters
    //console.log("command_added", ircEvent);


    ircEvent.channel = toBeParsed.split(" ").shift();
    toBeParsed = toBeParsed.slice(ircEvent.channel.length + 1); // removed the processed characters
    //console.log("channel_added", ircEvent);


    ircEvent.content = toBeParsed.split("\r\n").shift();
    toBeParsed = toBeParsed.slice(ircEvent.content.length + 2); // removed the processed characters
    //console.log("userdata_added", ircEvent);

    console.log("eventProcessed", ircEvent);
    console.log("REMAINING TO BE PARSED",toBeParsed);
