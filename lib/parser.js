...
module.exports = {
  ...
  format__general(event) {
    ...
    */
    ...
    ircEvent.tagString = ircEvent.hasTags ? ircEvent.toBeParsed.split(' :').shift() : undefined;
    ircEvent.toBeParsed = ircEvent.toBeParsed.slice(ircEvent.tagString.length + 1); // removed the processed tag characters
    //console.log("tags_added", ircEvent);

    ircEvent.clientPrefix = ircEvent.toBeParsed.split(" ").shift();
    ircEvent.toBeParsed = ircEvent.toBeParsed.slice(ircEvent.clientPrefix.length + 1); // removed the processed characters
    //console.log("prefix_added", ircEvent);


    ircEvent.command = ircEvent.toBeParsed.split(" ").shift();
    ircEvent.toBeParsed = ircEvent.toBeParsed.slice(ircEvent.command.length + 1); // removed the processed characters
    //console.log("command_added", ircEvent);


    ircEvent.channel = ircEvent.toBeParsed.split(" ").shift();
    ircEvent.toBeParsed = ircEvent.toBeParsed.slice(ircEvent.channel.length + 1); // removed the processed characters
    //console.log("channel_added", ircEvent);


    ircEvent.user = ircEvent.toBeParsed.split("\r\n").shift();
    ircEvent.toBeParsed = ircEvent.toBeParsed.slice(ircEvent.user.length + 1); // removed the processed characters
    //console.log("userdata_added", ircEvent);

    if(ircEvent.tagString) {
      ircEvent.tagString = ircEvent.tagString.substr(1);      // drop the first character "@"
      ircEvent.tagString = ircEvent.tagString.split(';');     // turn the object from a string to an array
      ircEvent.tagString.forEach((tag, idx) => {
        const splitTag = tag.split('=');
        const key = splitTag[0];
        const val = splitTag[1];
        ircEvent.tags[key] = val;
        console.log(`tags[${key}]=${val} from `, tag);
      });
