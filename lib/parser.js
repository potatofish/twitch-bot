...
function divideArgs(aCommand, argString) {
  ...
  switch (aCommand) {
    ...
    case '366': // > :<user>.tmi.twitch.tv 366 <user> #<channel> :End of /NAMES list
      ...
      args.channel = endOfNamesArgs.shift();
      args.notice = endOfNamesArgs.join(' ').slice(1);
      break;
    ...
    case "CAP":
      ...
      args.capabilities = capArgs.join(' ');
      break;
  ...
  format__general(event) {
    ...
    */
    function format__line(message) {
      ...
      if (`:${serverDomain}` !== clientPrefixRAW &&
        (ircMessage.command === 'PRIVMSG' || ircMessage.command === 'WHISPER')) {
        var bangSliceArray = clientPrefixRAW.slice(1).split("!");
        var userStrings = [];
        userStrings.push(bangSliceArray.shift());

        var atSliceArray = bangSliceArray.join('!').split('@');
        userStrings.push(atSliceArray.shift());

        var dotSliceArray = atSliceArray.join('@').split('.');
        userStrings.push(dotSliceArray.shift());

        var userDomain = dotSliceArray.join('.');

        //validate clientPrefix
        if (userDomain !== serverDomain ||
            userStrings[0] !== userStrings[1] ||
            userStrings[0] !== userStrings[2]) {
            throw `Invalid Format for clientPrefix:${clientPrefixRAW}`;
        }
        ircMessage.clientPrefix = {
