...
function divideArgs(aCommand, argString) {
  ...
  switch (aCommand) {
    ...
    case '353': // > :<user>.tmi.twitch.tv 353 <user> = #<channel> :<user1> <user2> ... <userN>
      ...
      undefined = namesArgs.shift(); // remove the "=" element
    ...
    // handled
    default:
       console.log(`Unknown Command "${aCommand}": ${argString}`);
  }
  //console.log(args);
  return args;
}
  ...
  format__general(event) {
    ...
    */
    function format__line(message) {
      ...
      if (ircMessage.clientPrefix === "PING") {
        const tmpCP = ircMessage.clientPrefix;
        const tmpCMD = ircMessage.command;
        ircMessage.command = tmpCP;
        ircMessage.clientPrefix = tmpCMD;
      }
    ...
    event.split("\r\n").forEach((ele, idx) => {
      if (ele.length > 0)
        // all the logic needs to go in here instead
