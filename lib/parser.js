  ...
  switch (aCommand) {
    ...
    case 'JOIN':
      if (argString.charAt(0) !== '#')
        throw `Unexpected JOIN: ${argString}`;
      args.channel = argString.trim();
      break;
    case 'PRIVMSG':
      if (argString.charAt(0) !== '#')
        throw `Unexpected PRIVMSG: ${argString}`;
    ...
    case 'WHISPER':
        ...
        args.messageFull = whisArgs.join(' ');
        undefined = whisArgs.shift();
        args.messageContent = whisArgs.join(' ');
        break;
    default:
